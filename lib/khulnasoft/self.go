...
import (
	...
	"github.com/khulnasoft/arbiter/khulnasoft/users"
)
...
func getKhulnasoftOrg(auth *securityprovider.SecurityProviderApiKey) (*uuid.UUID, error) {
	experimental, err := users.NewClientWithResponses(khulnasoftServer, users.WithRequestEditorFn(auth.Intercept))
	if err != nil {
		return nil, err
	}
