...
func TestEnrichSBOM_CycloneDXWithVulnerabilities(t *testing.T) {
	...
	assert.Equal(t, "KHULNASOFT-PYTHON-NUMPY-73513", vuln.ID)
}
...
func setupTestEnv(t *testing.T) func() {
	...
	t.Setenv("KHULNASOFT_TOKEN", "asdf")

	httpmock.Activate()
	httpmock.RegisterResponder(
		"GET",
		`=~^https://api\.khulnasoft\.io/rest/self`,
		httpmock.NewJsonResponderOrPanic(200, httpmock.File("testdata/self.json")),
	)
	httpmock.RegisterResponder(
		"GET",
		`=~^https://api\.khulnasoft\.io/rest/orgs/[a-z0-9-]+/packages/pkg%3Apypi%2Fnumpy%401.16.0/issues`,
		httpmock.NewJsonResponderOrPanic(200, httpmock.File("testdata/numpy_issues.json")),
	)
	httpmock.RegisterResponder(
		"GET",
		`=~^https://api\.khulnasoft\.io/rest/orgs/[a-z0-9-]+/packages/.*/issues`,
